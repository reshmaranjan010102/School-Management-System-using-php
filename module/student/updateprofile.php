				...
				<form  action="" method="POST">
					<div class="container">
						<div class="row mb-2">
							...
							<input type="text" name="name" class="col-9 form-control" placeholder="<?php echo$row['name'];?>" required>
						</div>
						<div class="row mb-2">
						...
						<div class="row mb-2">
							...
							<input type="text" name="address" class="col-9 form-control" placeholder="<?php echo$row['address'];?>" required/>
						</div>
						<button class="btn btn-success float-right mb-3" name="submit" type="submit">Update</button>
					</div>
				</form>
			</div>
		</div>
...
<?php
	if (isset($_POST['submit'])) 
	{
		...
		$mob1 = $_POST['mob1'];
		mysqli_query($link, "UPDATE students set name='$name', dob='$dob', sex='$sex', address='$address', phone='$mob1' where email='$email';");
